...
copydesktop() {
    ...
    if [[ -f "$src" ]]; then
        cp "$src" "$dest"
        echo "Copied $1.desktop to $dest"
    else
        echo "Error: $1.desktop not found in /usr/share/applications/"
    fi
}

# Tab completion for copydesktop
_copydesktop_completion() {
    local cur
    cur="${COMP_WORDS[COMP_CWORD]}"
    COMPREPLY=( $(compgen -W "$(ls /usr/share/applications/*.desktop 2>/dev/null | sed 's|.*/||;s|\.desktop$||')" -- "$cur") )
}
complete -F _copydesktop_completion copydesktop
...
if [ -f '/home/aaravk/google-cloud-sdk/path.zsh.inc' ]; then . '/home/aaravk/google-cloud-sdk/path.zsh.inc'; fi
